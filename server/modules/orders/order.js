...
class Order extends BaseModule {
  ...
  async createOrder(req) {
    ...
    const order = new OrderModel({ orderId: (lastOrderId + 1), ...cart, orderItems: cart.cartItems, ...this.getDefaultCreateFields(req) });
  ...
  async getOrderList({ query }) {
    ...
    orderData.records.forEach((record, index)=>{
      ...
      orderData.records[index].orderItems.forEach((item, itemIndex)=>{
        orderData.records[index].orderItems[itemIndex] = { ...item, product: item.productId, productId: item.productId._id };
      });
    });
    return orderData;
  }
